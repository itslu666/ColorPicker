...
class CTkColorPicker(customtkinter.CTkFrame):
    ...
    def __init__(self,
        ...
        self.fg_color = self._apply_appearance_mode(
            self._fg_color) if fg_color is None else fg_color
        ...
        self.canvas = tkinter.Canvas(self, height=self.image_dimension,
                                     width=self.image_dimension, highlightthickness=0, bg=self.fg_color)
        self.canvas.bind("<B1-Motion>", self.on_mouse_drag)

        if ImageGrab.grabclipboard() == None:
            self.img1 = Image.open(os.path.join(PATH, image)).resize(
                (self.image_dimension, self.image_dimension), Image.Resampling.LANCZOS)
        else:
            self.img1 = ImageGrab.grabclipboard().resize(
                (self.image_dimension, self.image_dimension), Image.Resampling.LANCZOS)
        ...
        self.copy_button = customtkinter.CTkButton(
            master=self, text="Copy Color-Code", corner_radius=self.corner_radius, command=self.copy_color_code)

        if orientation == "vertical":
            self.canvas.pack(pady=20, side="left", padx=(10, 0))
            self.slider.pack(fill="y", pady=15, side="right",
                             padx=(0, 10-self.slider_border))
            self.label.pack(expand=True, fill="both", padx=10, pady=15)
            self.copy_button.pack(fill="x", pady=(0, 20), padx=10)
        else:
            self.label.configure(wraplength=100)
            self.canvas.pack(pady=15, padx=15)
            self.slider.pack(fill="x", pady=(
                0, 10-self.slider_border), padx=15)
    ...
    def on_mouse_drag(self, event):
        ...
        d_from_center = math.sqrt(
            ((self.image_dimension/2)-x)**2 + ((self.image_dimension/2)-y)**2)
        ...
        self.canvas.create_image(
            self.target_x, self.target_y, image=self.target)
    ...
    def set_initial_color(self, initial_color):
        ...
        if initial_color and initial_color.startswith("#"):
            try:
                r, g, b = tuple(int(initial_color.lstrip(
    ...
    def copy_color_code(self):
        self.master.clipboard_append(self.label.cget("text"))
