...
class CTkColorPicker(customtkinter.CTkFrame):
    ...
    def __init__(self,
        ...
        if ImageGrab.grabclipboard() == None:
            self.img1 = Image.open(os.path.join(PATH, image)).resize((self.image_dimension, self.image_dimension), Image.Resampling.LANCZOS)
        else:
            self.img1 = ImageGrab.grabclipboard().resize((self.image_dimension, self.image_dimension), Image.Resampling.LANCZOS)
        ...
        self.copy_button = customtkinter.CTkButton(master=self, text="Copy Color-Code", corner_radius=self.corner_radius, command=self.copy_color_code)
